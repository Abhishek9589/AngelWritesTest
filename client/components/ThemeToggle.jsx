...
export const BOOK_PRESETS = POEM_PRESETS;
...
function getContextPreset(context) {
  const key = `themePreset_${context}`;
  const stored = localStorage.getItem(key);
  const validKeys = (context === "book" ? BOOK_PRESETS : POEM_PRESETS).map((p) => p.key);
...
export function ThemeToggle() {
  ...
  useEffect(() => {
    ...
    const contexts = ["poem", "book"];
    const legacySingle = localStorage.getItem("themePreset");
    const legacyLight = localStorage.getItem("themePresetLight");
    const legacyDark = localStorage.getItem("themePresetDark");

    contexts.forEach((ctx) => {
      const unifiedKey = `themePreset_${ctx}`;
      if (!localStorage.getItem(unifiedKey)) {
        const perLight = localStorage.getItem(`themePresetLight_${ctx}`);
        const perDark = localStorage.getItem(`themePresetDark_${ctx}`);
        const chosen = (legacySingle || (system === "dark" ? perDark : perLight) || perLight || perDark || "pastel");
